...
% 	disp(sprintf('%d   , obj: %f, RMS: %f, L-S: %d', k,f1,RMSerr,lsiter));
    ...
    norm_val = norm(dx(:));
    gradToll2 = 5e-3;
    norm_diff = abs(norm_val - prev_norm_val);
    if ~isdeployed
         %if (norm_diff > gradToll)
         if (norm_diff > gradToll2)
            msg = ['Iteration: ' num2str(k-1) ': Convergence metric is ' num2str(norm_diff) ', which is still greater than the convergence criteria, ' num2str(gradToll2) '.'];
            disp(msg)
         else
             msg = ['Iteration: ' num2str(k-1) ': Convergence metric is ' num2str(norm_diff) '-- CONVERGENCE LIMIT HAS BEEN MET!'];
             disp(msg)
         end
    end
	if (k > 10*params.Itnlim) | (norm_diff < gradToll2) 
		break;
	end
