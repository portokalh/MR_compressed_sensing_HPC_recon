function CSDTI_recon_on_cluster_TESTER_scratch(scanner,runno,study,series,target_machine,fermi_filter,CS_recon_params,chunk_size)
...
if ((nechoes > 1) && (nvols == 1))
    nvols = nechoes;   
end
dependent_jobid='0'; % Need for multi-echo, should be ignored for DTI, etc.
for k = 1:nvols
    %CS_recon_cluster_bj_multithread(reconfile,k,workpath,scale_file,target_machine,chunk_size);
   dependent_jobid = CS_recon_cluster_bj_multithread_v2a_scratch(reconfile,k,recon_path,scale_file,target_machine,fermi_filter,chunk_size,CS_recon_params,dependent_jobid); %v2
   dependent_jobid
end
