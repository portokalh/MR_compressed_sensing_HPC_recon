function savefast2(filename,varargin)
%function savefast2(filename, append_flag,varargin) % attempted to adapt it
%   such that it had append support.
...
% Copyright 2013 by Timothy E. Holy
  ...
  if all(isnum)
    ...
    dummy = 0; %#ok<NASGU>
    %if append_flag
    %    save(filename, '-v7.3', 'dummy', '-append');
    %else
   ...
   % if append_flag
   %     save(filename, '-v7.3', '-struct', 's','-append');
   % else
